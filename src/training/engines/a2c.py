"""Readable fixed-rollout training loop for A2C with GAE."""
...
class A2CTrainingEngine:
    """
    Train A2C with a visible fixed-rollout agent-environment loop.

    In accordance with `docs/LEARNING.md`, a rollout may cross episode
    boundaries. Each transition stores current and next critic values, while
    the agent computes detached GAE advantages and critic targets when the
    rollout reaches its configured size. A final shorter rollout is also used.

    Fields:
        * agent: A2C actor, critic, and separate optimizers.
    ...
    def train(
        ...
        *,
        ...
        # The last stored transition is an explicit GAE recursion boundary, so a
        # shorter final rollout has a complete target definition.
